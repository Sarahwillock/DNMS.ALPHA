      ...
      <div className="mx-auto max-w-7xl">
                ...
                className={`${
                  event.color === 'move-orange'
                    ...
                    : event.color === 'white'
                      ? 'bg-white'
                      : 'bg-move-blue'
                } group/item flex flex-1 flex-col justify-center border-b-2 border-black p-8`}
              >
                <div
                  className={`${
                    event.color === 'white' ? 'text-black' : 'text-white'
                  } mb-4 text-6xl font-black tracking-tighter`}
                >
                  {event.date.split('/')[0]}
                </div>
                ...
                <div className="flex items-end justify-between gap-4">
                  <a
                    ...
                    } block border-l-4 pl-4 text-sm font-bold uppercase leading-relaxed tracking-tight transition-colors hover:text-move-pink`}
                  >
                    {event.title} - {event.day} · {event.time.split(' ')[0]} ·
                    {' '}Prédio da Igreja
                  </a>

                  <button
                    onClick={() => saveToCalendar(event)}
                    className={`ml-2 px-3 py-1 text-[8px] font-black uppercase tracking-tighter transition-all hover:scale-105 active:scale-95 ${
                      event.color === 'white'
        ...
        <div className="mt-8 grid grid-cols-1 gap-8 md:grid-cols-3">
          ...
          <div className="neo-brutalist-shadow group flex flex-col justify-between border-4 border-black bg-move-pink p-8 text-white transition-all hover:-translate-x-1 hover:-translate-y-1 hover:shadow-[8px_8px_0px_0px_rgba(0,0,0,1)]">
            ...
            <div>
              <h4 className="mb-3 text-3xl font-black uppercase leading-none">
                ACESSO VIP
              </h4>
